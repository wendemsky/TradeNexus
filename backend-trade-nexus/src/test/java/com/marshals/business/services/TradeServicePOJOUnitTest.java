...
public class TradeServicePOJOUnitTest {
	...
	public void setUp() throws Exception {
		...
		mockFIPSService = mock(FIPSService.class);
		Mockito.when(mockFIPSService.getLivePrices()).thenReturn(priceList);

		service = new TradeService(mockDao, mockPortfolioService, mockFIPSService);
...
//		assertEquals("Instrument is not present in the platform", e.getMessage());
	...
	public void testExecuteSuccessfulBuyTrade() {
		...
		Mockito.when(mockFIPSService.createTrade(order)).thenReturn(trade1);
	...
	public void testExecuteBuyTradeWithInsufficientBalanceThrowsException() {
		...
		Mockito.when(mockFIPSService.createTrade(order)).thenReturn(trade);
		Exception e = assertThrows(NullPointerException.class, () -> {
			service.executeTrade(order);
		});
