...
class ClientServicePOJOUnitTest {
	...
	@Mock FIPSService mockFIPSService;
	...
	void setUp() throws Exception {
		...
		//Initializing the Client Service with a Mock Dao and mock fips service
	...
	@Test
	void shouldHandleFIPSInvalidationAtRegistration() {
		String newEmail = clientList.get(2).getEmail();
		//Mock the behavior of Dao method to throw exception - For email validation to return false
		Mockito.doThrow(new DatabaseException()).when(mockDao).verifyClientEmail(newEmail);
		// Mock behavior of Dao method that retrieves all client ID Details
		Mockito.when(mockDao.getAllClientIdentificationDetails()).thenReturn(clientList.get(0).getIdentification());
		//Mocking fipsService to throw FIPSException - Invalidate client
		Mockito.doThrow(new FIPSException()).when(mockFIPSService).verifyClient(newEmail);
		...
		assertThrows(FIPSException.class, () -> {
			...
			Mockito.verify(mockDao).getAllClientIdentificationDetails(); 
			//Verifying that mock fips serivce was called
			Mockito.verify(mockFIPSService).verifyClient(newEmail);
		});
	...
	void testSuccesfulRegistrationOfClientWithValidDetails() {
		...
		// Mock behavior of Dao method that retrieves all client ID Details
		Mockito.when(mockDao.getAllClientIdentificationDetails()).thenReturn(clientList.get(0).getIdentification());
		//Mocking fipsService to also successfully validate the client
		Mockito.when(mockFIPSService.verifyClient(newEmail)).thenReturn(
				new FIPSValidatedClient(clientList.get(2).getClientId(),newEmail,767836496));
		//New Client Details
	...
	@Test
	void shouldHandleFIPSInvalidationAtLogin() {
		String existingEmail = clientList.get(0).getEmail();
		...
		//Mocking fipsService to throw FIPSException - Invalidate client
		Mockito.doThrow(new FIPSException()).when(mockFIPSService).verifyClient(existingEmail, clientList.get(0).getClientId());
	...
	void testSuccesfulLoginOfExistingValidClient() {
		...
		Mockito.when(mockDao.getClientAtLogin(existingEmail,validPassword)).thenReturn(clientList.get(0));
		//Mocking fipsService to also successfully validate the client
		Mockito.when(mockFIPSService.verifyClient(existingEmail, clientList.get(0).getClientId())).
				thenReturn(new FIPSValidatedClient(clientList.get(0).getClientId(), existingEmail, 1654658069));
