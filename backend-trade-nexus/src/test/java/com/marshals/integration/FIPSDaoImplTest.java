...
class FIPSDaoImplTest {
	...
	void testForValidatationOfNullClientEmailAtRegistrationThrowsException() {
		...
		assertEquals("FIPS couldnt validate new Client's Email",e.getMessage());
	}
	@Test
	...
	void testForSuccessfulValidatationOfClientEmailAtRegistration() {
		FIPSValidatedClient validatedClient = dao.verifyClient(newClientEmail);
	...
	void testForSuccessfulValidatationOfClientCredentialsAtLogin() {
		FIPSValidatedClient validatedClient = dao.verifyClient(existingClientEmail,existingClientID);
		assertNotNull(validatedClient);
