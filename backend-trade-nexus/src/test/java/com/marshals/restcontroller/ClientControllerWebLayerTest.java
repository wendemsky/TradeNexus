...
//		System.out.println("JSON: "+jsonString);
	...
	@Test
	void testWhenServiceThrowsFIPSExceptionAtRegistration_RespondsWith400() throws Exception {
		Client client = clientList.get(0);
		SimpleDateFormat dateFormat = new SimpleDateFormat("MM/dd/yyyy"); //doB Format stored in client model
		String dateOfBirth = dateFormat.format(client.getDateOfBirth());
		
		// Mocking service to throw a FIPSException
		when(mockService.registerNewClient(client.getEmail(), client.getPassword(), client.getName(), 
				dateOfBirth, client.getCountry(), client.getIdentification())).thenThrow(new FIPSException()); 
	...
	@Test
	void testWhenServiceThrowsFIPSExceptionAtLogin_RespondsWith400() throws Exception {
		String email = clientList.get(1).getEmail();
		String password = clientList.get(1).getPassword(); 
		// Mocking service to throw FIPSException
		when(mockService.loginExistingClient(email, password)).thenThrow(new FIPSException("Couldnt validate client")); 
