...
import com.marshals.business.services.FIPSService;
...
public class TradeController {
	...
	// Get live instrument prices - from fips - Directly call fipsService
	...
	public ResponseEntity<List<Price>> getLivePrices(){
		...
		try {
			List<Price> priceList = fipsService.getLivePrices();
	...
	public ResponseEntity<Trade> executeTrade(@RequestBody Order order) {
		...
		try {
			...
			logger.error("FIPS Exception while executing trade", e);
			 throw new ResponseStatusException(HttpStatus.BAD_REQUEST, e.getLocalizedMessage());
		} catch (RuntimeException e) {
