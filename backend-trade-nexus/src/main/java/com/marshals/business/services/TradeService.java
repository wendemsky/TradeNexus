...
public class TradeService {
	...
	public TradeService(@Qualifier("clientTradeDao") ClientTradeDao dao, @Qualifier("portfolioService") PortfolioService portfolioService,  @Qualifier("fipsService") FIPSService fipsService) {
...
//		this.portfolioService = new PortfolioService(dao); // For getting and updating client portfolio
		...
		priceList = fipsService.getLivePrices(); //Get Live Prices from FIPSService
	}
        	...
        	if(order.getDirection().equals("B")) {
        		for(Price price: priceList) {
            		if(price.getInstrument().getInstrumentId().equals(order.getInstrumentId())) {
            			//Call FIPS Service to create the trade
            			Trade trade;
    					
						trade = fipsService.createTrade(order);
