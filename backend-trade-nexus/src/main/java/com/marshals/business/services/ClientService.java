...
public class ClientService {
	...
	@Autowired
	public ClientService(@Qualifier("clientDao") ClientDao dao, @Qualifier("fipsService") FIPSService fipsService) {
	...
	public LoggedInClient registerNewClient(String email, String password, String name,
		...
		try {
			...
			//Validating Client with FIPS
			FIPSValidatedClient validatedClient = fipsService.verifyClient(email); 
	...
	public LoggedInClient loginExistingClient(String email, String password) {
		try {
			...
			//Validating Existing Client with FIPS
			FIPSValidatedClient validatedClient = fipsService.verifyClient(email,existingClient.getClientId()); 
			if(validatedClient == null)
