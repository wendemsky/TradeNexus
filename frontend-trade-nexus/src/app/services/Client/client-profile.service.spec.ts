...
describe('ClientProfileService', () => {
  ...
  // it('fips should verify client data successfully', () => {
  ...
  //   service.fipsClientVerification(clientData).subscribe(data => { validResponse = data });

  //   const req = httpTestingController.expectOne(service.fipsURL);
  ...
  // //Test to check if our service handles a fips 406 error - TESTING FOR FAILURES
  // it('should handle a 406 error from fips service', inject([ClientProfileService],
  ...
  //     const errorHandlerSpy = spyOn(service, 'handleFIPSError')
  //       .and.callThrough();
  //     service.fipsClientVerification(expectedClientData) //Trying to validated a client data
  //       .subscribe({
  //         next: () => fail('Should fail'),
  //         error: (err) => errorReply = err
  //       });
  //     const req = httpTestingController.expectOne(service.fipsURL); //Checking if url is right
